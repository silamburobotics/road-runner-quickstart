...
@Disabled
@Config
@TeleOp(name = "TeleOpDECODEbooster", group = "TeleOp")
public class TeleOpDECODEbooster extends LinearOpMode {
    ...
    private ElapsedTime boostTimer = new ElapsedTime(); // Timer for voltage boost duration
    private boolean boostActive = false; // Track if voltage boost is active
    ...
    public static double VELOCITY_P = 3.0;   // Lower P for brushed motor (smoother response)
    public static double VELOCITY_I = 0.5;   // Higher I to maintain speed during load
    public static double VELOCITY_D = 0.25;   // Moderate D for damping
    public static double VELOCITY_F = 16.0;  // Higher F for better feedforward at high RPM

    // Speed monitoring thresholds - Relaxed for brushed motor characteristics
    public static final double SHOOTER_SPEED_THRESHOLD = 0.92;      // 90% instead of 95%
    public static final double SHOOTER_MIN_SPEED_THRESHOLD = 0.88;  // Keep at 85%
    public static final double SHOOTER_SPEED_TOLERANCE = 75;       // Wider tolerance for brush noise (100 ticks)
    public static final double SHOOTER_STABILIZATION_TIME = 0.5;    // Faster stabilization (0.4s instead of 1.0s)
    
    // Voltage boost settings for faster spin-up
    public static final double BOOST_VOLTAGE_MULTIPLIER = 1.25;      // 30% overspeed during boost
    public static final double BOOST_DURATION = 0.25;                // 300ms boost duration
    ...
    private void toggleShooterSpeed(double velocity) {
        if (shooterRunning && Math.abs(currentShooterVelocity - velocity) < 50) {
            ...
            double boostedVelocity = velocity * BOOST_VOLTAGE_MULTIPLIER;
            ...
            telemetry.addData("🎯 Shooter STARTED", "%.0f ticks/sec (BOOST: %.0f)", velocity, boostedVelocity);
    ...
    private void updateShooterSpeedMonitoring() {
        ...
        if (boostActive && boostTimer.seconds() >= BOOST_DURATION) {
            ...
            double boostedVelocity = currentShooterVelocity * BOOST_VOLTAGE_MULTIPLIER;
            shooter.setVelocity(boostedVelocity);
